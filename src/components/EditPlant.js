...
export default function EditPlant(props) {
  ...
  const formSchema = yup.object().shape({
    nickname: yup.string().required(2, "Nickname is a required field."),
    species: yup.string().required("Must include plant species."),
    h2o_frequency: yup
      .number()
  ...
  // Submit button is disabled until form data is valid
  ...
  // Submit changes to database and push user to PlantCard
  ...
  // Delete this particular plant & it's values
  const handleDelete = () => {
  ...
  // Updates local state while form values change
  const inputChange = (e) => {
    e.persist();
    setValues({
      ...values,
